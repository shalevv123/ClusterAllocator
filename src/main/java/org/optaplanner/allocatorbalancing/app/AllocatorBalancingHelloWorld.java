...
public class AllocatorBalancingHelloWorld {

    public static void main(String[] args) throws IOException, SAXException, ParserConfigurationException {
        // Build the Solver
        AllocatorBalance unsolvedAllocatorBalance = getBalance("300clusters-2400servers.xml");
        ...
        List<Pair<SolverFactory<AllocatorBalance>,Integer>> solvers = new ArrayList<>();
        for(int i=5 ; i<=105 ; i+=10) {
            SolverFactory<AllocatorBalance> solverFactory = SolverFactory.create(new SolverConfig()
                    ...
                    .withTerminationSpentLimit(Duration.ofSeconds(i)));
            solvers.add(new MutablePair<>(solverFactory,i));
        }

        List<Pair<AllocatorBalance,Integer>> solvedList = new ArrayList<>();
        for (Pair<SolverFactory<AllocatorBalance>,Integer> solverFactory : solvers) {
            Solver<AllocatorBalance> solver = solverFactory.getKey().buildSolver();
            AllocatorBalance solvedAllocatorBalance = solver.solve(unsolvedAllocatorBalance);
            solvedList.add(new MutablePair<>(solvedAllocatorBalance,solverFactory.getValue()));
        }

            // Display the result //

        System.out.println("\nSolved allocatorBalance with 300 clusters and 2400 servers:\n");
        for (Pair<AllocatorBalance,Integer> solvedAllocatorBalance : solvedList) {
            System.out.println(
                    "\nSolution ran for " + solvedAllocatorBalance.getValue() + " seconds\n"
                    + "Score is: "+ toDisplayString(solvedAllocatorBalance.getKey())+ "\n");
        }
        for (Pair<AllocatorBalance,Integer> solvedAllocatorBalance : solvedList) {
            System.out.println(
                    "\nSolution ran for " + solvedAllocatorBalance.getValue() + " seconds\n"
                    + "Score is: "+ solvedAllocatorBalance.getKey().getScore() + "\n");
        }
        for (Pair<AllocatorBalance,Integer> solvedCloudBalance : solvedList) {
            saveSolution(solvedCloudBalance.getKey(),"test-"+solvedCloudBalance.getValue()+"SecondsRuntime.xml");
            //saveSolution(solvedCloudBalance.getKey(),"test.xml");
        }
        XYSeries series = new XYSeries("Runtime Data");
        for (Pair<AllocatorBalance,Integer> solvedCloudBalance : solvedList) {
            series.add(solvedCloudBalance.getValue(),Double.valueOf(-solvedCloudBalance.getKey().getScore().getSoftScore()));
        }
        graphResults(series,"graph");
    ...
    public static void saveSolution(AllocatorBalance solution, String filename){
    ...
    private static void graphResults(XYSeries series, String filename){
